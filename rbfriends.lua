...
local Window =
    OrionLib:MakeWindow(
    {
        Name = "Blaze Hub - " .. CurrentGame,
        HidePremium = false,
        SaveConfig = true,
        IntroEnabled = false,
        ConfigFolder = "RealityHUB"
    }
)
...
Farming:AddButton(
    {
        ...
        Callback = function()
            game.Players.LocalPlayer.Character.HumanoidRootPart.CFrame =
                CFrame.new(-399.289794921875, 18.798019409179688, 645.77490234375)
        end
    }
)
...
function blueESP()
    if
        not game:GetService("Workspace").Monsters.Blue:FindFirstChild("Highlight") and
            game:GetService("Workspace").Monsters.Blue
     then
...
function greenESP()
    if
        not game:GetService("Workspace").Monsters.Green:FindFirstChild("Highlight") and
            game:GetService("Workspace").Monsters.Green
     then
...
function orangeESP()
    if
        not game:GetService("Workspace").Monsters.Orange:FindFirstChild("Highlight") and
            game:GetService("Workspace").Monsters.Orange
     then
